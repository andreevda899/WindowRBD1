...
{
    public partial class Measuring1 : System.Windows.Window
    {
        ...
        private void CmTab1()
        {
            if (cmbTab.Text == "Номер измерительного оборудования")
            {
                str3 = "[Номер измерительного оборудования]";
            }
        ...
        private void MeasuringCommand() //Происходит заполнение данными datagrid
        {
            sql = "select * from Proekt.[Измерительное оборудование]";
            uri = 13;
            cmbTab.ItemsSource = new string[] { "Номер измерительного оборудования", "Наименование" };
            RoleBox = "Proekt.[Измерительное оборудование]";
            using (SqlConnection conn = new SqlConnection(con))
            {
                conn.Open();
                SqlCommand myCommand = new SqlCommand();
        ...
        private void DeleteCommand_Click(object sender, RoutedEventArgs e) //Происходит удаление записи из базы данных
        {
 		    if (txtBox1.Text != "")
  		    ...
  		    {
      			    MessageBox.Show("Введите в поисковик Номер измерительного оборудования для удаления");
  		    }
        }
        ...
        private void dataGridView1_SelectionChanged(object sender, System.Windows.Controls.SelectionChangedEventArgs e) //Происходит выделение строки и заполнение информацией элементов
        {
            ...
            {
                DataRowView rowView = dataGridView1.SelectedValue as DataRowView;
                string str = "Select * from Proekt.[Измерительное оборудование]  where [Номер измерительного оборудования]  = " + rowView[0].ToString();
        ...
        {
            string path = "C:\\Users\\Пользователь\\Desktop\\WindowRBD1\\ExportToTxt\\exportMeasuring.txt";
            StreamWriter sw = new StreamWriter(path);
            DataTable dt = Select1("Select * from Proekt.[Измерительное оборудование]");
            for (int i = 0; i < dt.Rows.Count; i++)
            {
                sw.WriteLine("[Номер измерительного оборудования]:" + dt.Rows[i][0].ToString());
        ...
        private void btReport_Click(object sender, RoutedEventArgs e) // Экспорт данных из datagid в txt
        {
            if (txtBox1.Text != "")
            {
                new ReportMeasuring(txtBox1.Text).ShowDialog();
            }
            else
            {
                MessageBox.Show("Введите в поисковик Номер измерительного оборудования");
            }
        }
    }
}
