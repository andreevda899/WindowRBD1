...
{
    public partial class Area : System.Windows.Window
    {
        public Area()
        {
            InitializeComponent();
        ...
        private void ExportToTxt_Click(object sender, RoutedEventArgs e) // Экспорт данных из datagid в txt
        {
            ...
            for(int i = 0;i< dt.Rows.Count; i++)
            {
                ...
                sw.WriteLine("[Длина периметра]:" + dt.Rows[i][3].ToString());
                sw.WriteLine("[Величина площади окружённая периметром]:" + dt.Rows[i][4].ToString());
                sw.WriteLine("[Дата и время окончания работ]:" + dt.Rows[i][5].ToString());
                sw.WriteLine("[Дата и время появления записи]:" + dt.Rows[i][6].ToString());
                sw.WriteLine("[Дата и время изменения записи]:" + dt.Rows[i][7].ToString());
                sw.WriteLine("[Супервайзер полевых работ]:" + dt.Rows[i][8].ToString());
                sw.WriteLine("[Супервайзер обработки данных]:" + dt.Rows[i][9].ToString());
                sw.WriteLine("");
            }
            sw.Close();
