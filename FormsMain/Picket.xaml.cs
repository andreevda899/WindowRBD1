...
{
    public partial class Picket : System.Windows.Window
    {
        public Picket()
        {
            InitializeComponent();
        ...
        private void ExportToTxt_Click(object sender, RoutedEventArgs e) // Экспорт данных из datagid в txt
        {
            ...
            for(int i = 0;i< dt.Rows.Count; i++)
            {
                ...
                sw.WriteLine("[Номер отряда]:" + dt.Rows[i][4].ToString());
                sw.WriteLine("[Номер методики]:" + dt.Rows[i][5].ToString());
                sw.WriteLine("[Номер оборудования]:" + dt.Rows[i][6].ToString());
