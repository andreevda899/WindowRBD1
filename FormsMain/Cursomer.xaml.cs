...
{
    public partial class Cursomer : System.Windows.Window
    {
        public Cursomer()
        {
            InitializeComponent();
        ...
        private void DeleteCommand_Click(object sender, RoutedEventArgs e) //Происходит удаление записи из базы данных
        {
 		    if (txtBox1.Text != "")
  		    ...
  		    {
      			    MessageBox.Show("Введите в поисковик Номер заказчика для удаления");
  		    }
        }
        ...
        private void dataGridView1_SelectionChanged(object sender, System.Windows.Controls.SelectionChangedEventArgs e)//Происходит выделение строки и заполнение информацией элементов
        {
            ...
            if (flag == false)
            {
                DataRowView rowView1 = dataGridView1.SelectedValue as DataRowView;
                tr = rowView1[0].ToString();


                string str = "Select * from Proekt.Заказчик where [Номер заказчика] = " + rowView1[0].ToString();
        ...
        private void btReport_Click(object sender, RoutedEventArgs e) // Экспорт данных из datagid в txt
        {
            ...
            {
                MessageBox.Show("Введите в поисковик Номер заказчика");
            }
        }
    }
