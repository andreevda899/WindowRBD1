...
{
    public partial class Engineer: System.Windows.Window
    {
	    public Engineer()
        {
            InitializeComponent();
        ...
        private void EngineerCommand() //Происходит заполнение данными datagrid
        {
            sql = "select * from Proekt.ИТР";
            uri = 8;
            cmbTab.ItemsSource = new string[] { "Номер ИТР", "ФИО" };
            RoleBox = "Proekt.ИТР";
            using (SqlConnection conn = new SqlConnection(con))
            {
                conn.Open();
        ...
        private void DeleteCommand_Click(object sender, RoutedEventArgs e) //Происходит удаление записи из базы данных
        {
 		    if (txtBox1.Text != "")
  		    ...
  		    {
      			    MessageBox.Show("Введите в поисковик Номер ИТР для удаления");
  		    }
        }
        ...
        private void dataGridView1_SelectionChanged(object sender, System.Windows.Controls.SelectionChangedEventArgs e) //Происходит выделение строки и заполнение информацией элементов
        {
            ...
            if(flag == false) {
                DataRowView rowView = dataGridView1.SelectedValue as DataRowView;
                string str = "Select * from Proekt.[ИТР]  where [Номер ИТР] = " + rowView[0].ToString();
                ...
                using (SqlConnection conn = new SqlConnection(BdCon.Con))
                {
                    ...
                    da.Fill(dr);
                    txtNumberEngineer.Content = dr.Rows[0][0].ToString();
        ...
        {
            string path = "C:\\Users\\Пользователь\\Desktop\\WindowRBD1\\ExportToTxt\\exportEngineer.txt";
            StreamWriter sw = new StreamWriter(path);
            DataTable dt = Select1("Select * from Proekt.[ИТР]");
            for (int i = 0; i < dt.Rows.Count; i++)
            {
                sw.WriteLine("[Номер ИТР]:" + dt.Rows[i][0].ToString());
	    ...
	    if (txtBox1.Text != "")
            {
                new ReportITR(txtBox1.Text).ShowDialog();
            }
            else
            {
                MessageBox.Show("Введите в поисковик Номер ИТР");
            }
        }
    }
}
