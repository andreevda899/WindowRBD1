...
namespace WindowRBD1.FormsMain
{
    public partial class Driver : System.Windows.Window
    {
        ...
        private void DriverCommand() //Происходит заполнение данными datagrid
        {
            sql = "select * from Proekt.Водители";
            uri = 11;
            cmbTab.ItemsSource = new string[] { "Номер Водителя", "ФИО" };
            RoleBox = "Proekt.Водители";
            using (SqlConnection conn = new SqlConnection(con))
            {
                conn.Open();
                SqlCommand myCommand = new SqlCommand();
        ...
        {
        	CreateDriver form1 = new CreateDriver();
        	form1.Show();
        }
        ...
        {
        	EditDriver form1 = new EditDriver();
        	form1.Show();
        }
        ...
        private void DeleteCommand_Click(object sender, RoutedEventArgs e) //Происходит удаление записи из базы данных
        {
 		    if (txtBox1.Text != "")
  		    ...
  		    {
      			    System.Windows.MessageBox.Show("Введите в поисковик Номер Водителя для удаления");
  		    }
        }
        ...
        private void dataGridView1_SelectionChanged(object sender, System.Windows.Controls.SelectionChangedEventArgs e) //Происходит выделение строки и заполнение информацией элементов
        {
            ...
            {
                DataRowView rowView = dataGridView1.SelectedValue as DataRowView;
                string str = "Select * from Proekt.Водители  where [Номер Водителя] = " + rowView[0].ToString();
                ...
                using (SqlConnection conn = new SqlConnection(BdCon.Con))
                {
                    ...
                    da.Fill(dr);
                    txtNumberDriver.Content = dr.Rows[0][0].ToString();
        ...
        {
            string path = "C:\\Users\\Пользователь\\Desktop\\WindowRBD1\\ExportToTxt\\exportDriver.txt";
            StreamWriter sw = new StreamWriter(path);
            DataTable dt = Select1("Select * from Proekt.[Водители]");
            for (int i = 0; i < dt.Rows.Count; i++)
            {
                sw.WriteLine("[Номер Водителя]:" + dt.Rows[i][0].ToString());
	    ...
	    if (txtBox1.Text != "")
            {
                new ReportDriver(txtBox1.Text).ShowDialog();
            }
            else
            {
                System.Windows.MessageBox.Show("Введите в поисковик Номер Водителя");
            }
        }
    }  
}
