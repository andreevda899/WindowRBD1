...
namespace WindowRBD1.FormsMain
{
    public partial class Order : System.Windows.Window
    {
        ...
        private void СlearCommand_Click(object sender, RoutedEventArgs e) //Происходит заполнение данными datagrid
        {
            ...
            if (flag == true)
            {
                txtNumberOrder.Content = "";
                FioChief.Content = "";
                FioITR.Content = "";
                FioDriver.Content = "";
                FioWorker.Content = "";
                FioSupervisor.Content = "";
                flag = false;
            }
        }
        ...
        private void OrderCommand() //Происходит заполнение данными datagrid
        {
            sql = "select * from Proekt.[Полевой отряд]";
            uri = 7;
            cmbTab.ItemsSource = new string[] { "Номер отряда" };
            RoleBox = "Proekt.[Полевой отряд]";
            using (SqlConnection conn = new SqlConnection(con))
            {
                conn.Open();
                SqlCommand myCommand = new SqlCommand();
        ...
        private void DeleteCommand_Click(object sender, RoutedEventArgs e) //Происходит удаление записи из базы данных
        {
 		    if (txtBox1.Text != "")
  		    ...
  		    {
      			    MessageBox.Show("Введите в поисковик Номер отряда для удаления");
  		    }
        }
        ...
        private void dataGridView1_SelectionChanged(object sender, System.Windows.Controls.SelectionChangedEventArgs e) //Происходит выделение строки и заполнение информацией элементов
        {
            ...
            {
                DataRowView rowView = dataGridView1.SelectedValue as DataRowView;
                string str = "Select * from Proekt.[Полевой отряд]  where [Номер отряда] = " + rowView[0].ToString();
                ...
                using (SqlConnection conn = new SqlConnection(BdCon.Con))
                {
                    ...
                    txtNumberOrder.Content = dr.Rows[0][0].ToString();
        ...
        {
            string path = "C:\\Users\\Пользователь\\Desktop\\WindowRBD1\\ExportToTxt\\exportOrder.txt";
            StreamWriter sw = new StreamWriter(path);
            DataTable dt = Select1("Select * from Proekt.[Полевой отряд]");
            for(int i = 0;i< dt.Rows.Count; i++)
            {
                sw.WriteLine("[Номер отряда]:" + dt.Rows[i][0].ToString());
                sw.WriteLine("[Номер Начальника]:" + dt.Rows[i][1].ToString());
                sw.WriteLine("[Номер ИТР]:" + dt.Rows[i][2].ToString());
                sw.WriteLine("[Номер Водителя]:" + dt.Rows[i][3].ToString());
                sw.WriteLine("[Номер Рабочего] :" + dt.Rows[i][4].ToString());
                sw.WriteLine("[Номер Супервайзера]:" + dt.Rows[i][5].ToString());
                sw.WriteLine("");
            }
            sw.Close();
	    ...
	    if (txtBox1.Text != "")
            {
                new ReportOrder(txtBox1.Text).ShowDialog();
            }
            else
            {
                MessageBox.Show("Введите в поисковик Номер отряда");
            }
	}
    }
}
