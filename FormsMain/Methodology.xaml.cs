...
{
    public partial class Methodology : System.Windows.Window
    {
        public Methodology()
        {
            InitializeComponent();
        ...
        private void СlearCommand_Click(object sender, RoutedEventArgs e) //Происходит заполнение данными datagrid
        {
            ...
            if (flag == true)
            {
                ...
                NameMethodology.Content = "";
                txtGenerative.Content = "";
                txtMeasuring.Content = "";
                txtTelemetry.Content = "";
                txtImpulse.Content = "";
                txtPause.Content = "";
                txtCurrent.Content = "";
                flag = false;
        ...
        private void CmTab1()
        {
            if (cmbTab.Text == "Номер методики")
            {
                str3 = "[Номер методики]";
            }
   
            if (cmbTab.Text == "Наименование методики")
            {
                str3 = "[Наименование методики]";
            }
        }
        ...
        private void MethodologyCommand() //Происходит заполнение данными datagrid
        {
            sql = "select * from Proekt.Методика";
            uri = 16;
            cmbTab.ItemsSource = new string[] { "Номер методики", "Наименование методики" };
            RoleBox = "Proekt.Методика";
            using (SqlConnection conn = new SqlConnection(con))
            {
                conn.Open();
                SqlCommand myCommand = new SqlCommand();
                ...
                ds.Clear();
                dataAdapter.SelectCommand = myCommand;
                dataAdapter.Fill(ds, "Proekt.Методика"); 
                dataGridView1.ItemsSource = ds.Tables["Proekt.Методика"].DefaultView;
            } 
        }
        ...
        {
            CreateMethodology form1 = new CreateMethodology();
            form1.Show();
        }
        ...
        {
             EditMethodology form1 = new EditMethodology();
             form1.Show();
        }
        ...
        private void DeleteCommand_Click(object sender, RoutedEventArgs e) //Происходит удаление записи из базы данных
        {
 		    if (txtBox1.Text != "")
  		    ...
  		    {
      			    MessageBox.Show("Введите в поисковик Номер методики для удаления");
  		    }
        }
        ...
        private void dataGridView1_SelectionChanged(object sender, System.Windows.Controls.SelectionChangedEventArgs e) //Происходит выделение строки и заполнение информацией элементов
        {
            ...
            if (flag==false) { 
                DataRowView rowView = dataGridView1.SelectedValue as DataRowView;
                string str = "Select * from Proekt.Методика where [Номер методики] = " + rowView[0].ToString();
                ...
                using (SqlConnection conn = new SqlConnection(BdCon.Con))
                {
                    ...
                    NumberMethodology.Content = dr.Rows[0][1].ToString();
                    NameMethodology.Content = dr.Rows[0][1].ToString();
        ...
        {
            string path = "C:\\Users\\Пользователь\\Desktop\\WindowRBD1\\ExportToTxt\\exportMethodology.txt";
            StreamWriter sw = new StreamWriter(path);
            DataTable dt = Select1("Select * from Proekt.Методика");
            for (int i = 0; i < dt.Rows.Count; i++)
            {
                sw.WriteLine("[Номер методики]:" + dt.Rows[i][0].ToString());
                sw.WriteLine("[Наименование методики]:" + dt.Rows[i][1].ToString());
                sw.WriteLine("[Номер описания ген.установки]:" + dt.Rows[i][2].ToString());
                sw.WriteLine("[Номер описания измер.установки]:" + dt.Rows[i][3].ToString());
                sw.WriteLine("[Описание телеметрической установки]:" + dt.Rows[i][4].ToString());
                sw.WriteLine("[Продолжительность импульса]:" + dt.Rows[i][5].ToString());
                sw.WriteLine("[Продолжительность паузы]:" + dt.Rows[i][6].ToString());
                sw.WriteLine("[Сила тока]:" + dt.Rows[i][7].ToString());
                sw.WriteLine("");
            }
            sw.Close();
	    ...
	    if (txtBox1.Text != "")
            {
                new ReportMethodology(txtBox1.Text).ShowDialog();
            }
            else
            {
                MessageBox.Show("Введите в поисковик Номер методики");
            }
        }
    }
}
