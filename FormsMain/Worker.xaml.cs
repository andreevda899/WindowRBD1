...
{
    public partial class Worker : System.Windows.Window
    {
        public Worker()
        {
            InitializeComponent();
        ...
        private void WorkerCommand() //Происходит заполнение данными datagrid
        {
            sql = "select * from Proekt.Рабочие";
            uri = 10;
            cmbTab.ItemsSource = new string[] { "Номер Рабочего", "ФИО" };
            RoleBox = "Proekt.Рабочие";
            using (SqlConnection conn = new SqlConnection(con))
            {
                conn.Open();
                SqlCommand myCommand = new SqlCommand();
        ...
        {
                CreateWorker form1 = new CreateWorker();
                form1.Show();
        }
        ...
        {
                EditWorker form1 = new EditWorker();
                form1.Show();
 	}
        ...
        private void DeleteCommand_Click(object sender, RoutedEventArgs e) //Происходит удаление записи из базы данных
        {
 		    if (txtBox1.Text != "")
  		    ...
  		    {
      			    MessageBox.Show("Введите в поисковик Номер Рабочего для удаления");
  		    }
        }
        ...
        private void dataGridView1_SelectionChanged(object sender, System.Windows.Controls.SelectionChangedEventArgs e) //Происходит выделение строки и заполнение информацией элементов
        {
            ...
            if (flag == false) {
                DataRowView rowView = dataGridView1.SelectedValue as DataRowView;
                string str = "Select * from Proekt.[Рабочие]  where [Номер Рабочего] = " + rowView[0].ToString();
                ...
                using (SqlConnection conn = new SqlConnection(BdCon.Con))
                {
                    ...
                    da.Fill(dr);
                    txtNumberWorker.Content = dr.Rows[0][0].ToString();
        ...
        {
            string path = "C:\\Users\\Пользователь\\Desktop\\WindowRBD1\\ExportToTxt\\exportWorker.txt";
            StreamWriter sw = new StreamWriter(path);
            DataTable dt = Select1("Select * from Proekt.[Рабочие]");
            for (int i = 0; i < dt.Rows.Count; i++)
            {
                sw.WriteLine("[Номер Рабочего]:" + dt.Rows[i][0].ToString());
	    ...
	    if (txtBox1.Text != "")
            {
                new ReportWorker(txtBox1.Text).ShowDialog();
            }
            else
            {
                MessageBox.Show("Введите в поисковик Номер Рабочего");
            }
	}
    }
}
